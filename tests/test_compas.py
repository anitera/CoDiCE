...
class TestCFSearch(unittest.TestCase):
    def setUp(self):
        ...
        self.config = Config("config/conf_compas.yaml")
        with open("config/constraints_compas.json", 'r') as file:
    ...
    def prepare_data(self):
        input=pd.read_csv('datasets/compas_preprocessed.csv',sep=',')
        ...
        target_name = "class"
        input[target_name] = input[target_name].apply(lambda x: 1 if x=="High" else 0)
        ...
        input.to_csv("datasets/compas_ordered_features.csv", index=False)
        ...
        with open("models/compas_logistic_model.pkl", 'wb') as file:
    ...
    def test_cf_search(self):
        ...
        import time
        self.data = Dataset(self.config.get_config_value("dataset"), "class")
        ...
        test_indices = pd.read_csv("datasets/compas_test_indices.csv", header=None).to_numpy().squeeze(1)
        print(test_indices)
        ...
        test_df[:100].to_csv("results/original100_compas_train_diffusion.csv", index=False)
        all_counterfactuals.to_csv("results/counterfactuals100_compas_train_diffusion.csv", index=False) 
        # If you also want to save the timing information to a file
        with open('results/compas100_timing_info_diffusion.txt', 'w') as f:
